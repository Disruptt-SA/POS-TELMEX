...
import posLogo from '../../assets/img/logo/telmex_logo.png';
...
const objSale = ref({
	...
	client: {
		internalid: 8652,
		companyname: "CECILIA MIRANDA SANCHEZ",
		denviodefecto: 14657,
		dfacturaciondefecto: ""
	},
	currency: "MXN",
...
const fecha_hora = ref(moment().locale("es-mx").format("LL"))
...
onMounted(async () => {
	...
	items_list.value = await itemsStore.getListItemsWithCategory(category_item.value)
	categories_obj.value = await itemsStore.getCategories()
	//items_list.value = await itemsStore.getListItems()
...
const handleAddMethodPayment = async() => {
	...
	let total_venta = Number(objSale.value.total)
...
const recalculateFormsPayments = () => {
	let total = 0
	let change_cash = 0
	...
	if(total > objSale.value.total){
		change_cash = total - objSale.value.total
	}else{
		change_cash = 0
	}
...
const handlePrintTicket = async () => {
	...
	if(!continue_tran){
		...
		if (respSale?.status == "error") {
			...
			payments_sale.value = {
				sale_id:respSale.data.internalid,
...
const handleSelectCategory = async(categoryId) => {
	loader_items.value = true
	category_item.value = categoryId
	items_list.value = await itemsStore.getListItemsWithCategory(category_item.value)
	loader_items.value = false
}
</script>
...
<template>
	<div class="flex bg-[#01a0c7] w-full p-4 pt-32 md:pt-24 lg:pt-32">
								...
								<div class="sm:-mx-6 lg:-mx-8">
									<div class="inline-block min-w-full py-0 sm:px-6 lg:px-8">
										<div class="overflow-x-auto">
											...
											<table class="min-w-full text-left text-2xl font-light bg-white">
												<thead
				...
				<div class="w-full px-2 rounded-lg lg:col-span-3 mt-5 lg:mt-0 infoSale lg:right-9 bg-white pb-2">
					<!-- <div class="flex rounded-lg mb-2 w-full items-center justify-center">
						<div class="mr-2 w-full text-center">
							...
							<input
								class="appearance-none w-full text-center bg-gray-50 text-gray-700 border border-gray-300 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500 shadow-sl"
								id="grid-last-name" type="text" :value="fecha_hora">
						</div>
					</div> -->
					...
					<div class="bg-[#4689a1] text-white p-3 rounded-lg mb-2 flex flex-wrap">
						<div class="w-full">
							<label class="block uppercase tracking-wide text-2xl font-bold mb-2" for="grid-last-name">
								Metodo de pago
					...
					<div class="flex flex-wrap p-3 rounded-lg mb-2 shadow-sl">
						<div class="w-full">
							<label class="block uppercase tracking-wide text-3xl font-bold mb-2 text-black"
								for="grid-last-name">
								Resumen
							</label>
							<p class="text-2xl text-black font-thin">Cantidad de Artículos</p>
							<p class="text-3xl font-semibold mb-2">{{ objSale.itemsquantity }}</p>
							<p class="text-2xl text-black font-thin">Subtotal</p>
							<p class="text-3xl font-semibold mb-2">${{ objSale.subtotal }}</p>
							...
							<p class="text-4xl text-black font-bold">Total</p>
							<p class="text-5xl font-semibold">${{ objSale.total }}</p>
						</div>
					</div>
					<div class="flex">
						<button
		...
		<div class="bg-[#ffffff] w-[100%] rounded-3xl shadow-2xl p-2 md:p-10 lg:p-10"
			...
			<button class="bg-[#00a0c6] p-1 float-right md:mt-[-20px] md:mr-[-20px] rounded-full"
				@click="handleShowTicket()">
				<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
					class="bi bi-x text-white font-black w-12 h-12" viewBox="0 0 16 16">
					<path
			...
			<div class="flex flex-col items-center mt-8" id="ticketsale">
				<div class="w-full md:wfull lg:w-[30%] bg-white shadowCard p-4 items-center text-center">
					...
					<div class="grid grid-cols-4 mb-2 font-extrabold">
						<p class="text-[14px] text-center"><strong>Artículo</strong></p>
						<p class="text-[14px] text-center"><strong>Cantidad</strong></p>
						<p class="text-[14px] text-center"><strong>Precio Unitario</strong></p>
						<p class="text-[14px] text-center"><strong>SubTotal</strong></p>
					</div>
					...
					<p class="text-lg"><strong>SubTotal:</strong> ${{ Number(objSale.subtotal).toFixed(2) }}</p>
					<p class="text-lg"><strong>Impuestos:</strong> ${{ Number(objSale.taxes).toFixed(2) }}</p>
					<p class="text-lg"><strong>Total:</strong> ${{ Number(objSale.total).toFixed(2) }}</p>
					<p class="text-lg"><strong>IVA Incluido</strong></p>
					<p class="text-xl mt-5">Muchas gracias por su compra <br> vuelva pronto</p>
			...
			<div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
				<div
					...
					<div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
						<button type="button"
							class="mt-3 inline-flex w-full justify-center rounded-md bg-[#468aa2] px-3 py-2 text-sm font-semibold text-white shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-[#4689a2d3] sm:mt-0 sm:w-auto"
