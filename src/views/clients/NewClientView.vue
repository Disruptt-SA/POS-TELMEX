...
const formClient = ref({
	...
	syncns: false,
	syncmsg: null,
	custentity_forma_pago: null,
	custentity_metodo_pago: null
});
...
const handleCreateClient = async () => {
	...
	const internalId = getRandomInt(5000000, 10000000);
	txtBtnSbe.value = "Guardando, espere por favor...";
	formClient.value.internalid = internalId;
	formClient.value.addresses = objAddress.value;
	formClient.value._drt_custom_regimen_fiscal = Number(formClient.value._drt_custom_regimen_fiscal)
	formClient.value._drt_custom_uso_cfdi = Number(formClient.value._drt_custom_uso_cfdi)

	if(formClient.value.companyname != ''){
		formClient.value.isperson = true
	}else{
		formClient.value.isperson = false
	}

	const respSaveClient = await customerStore.saveNewClient(formClient.value);
	console.log("respSaveClient", respSaveClient);
	if (respSaveClient.status === 200) {
		if(respSaveClient.data.code == 500){
			toast.open({
				message: `${respSaveClient.data.msg}`,
				type: 'error'
			})
		}else if(respSaveClient.data.code == 200){
			for (let index = 0; index < objAddress.value.length; index++) {
				const dataResp = respSaveClient.data;
				const internalIdAddress = getRandomInt(5000000, 10000000);
				const element = objAddress.value[index];
				console.log("element", element);
				element.customerid = Number(dataResp.internalid);
...
const handleAddNewAddress = () => {
	...
	switch (buttonAddAddress.value) {
		case "Agregar":
			objAddress.value.push(formAddress.value);
			...
			break;
		case "Actualizar":
			objAddress.value[indexUpdateAddress.value] = formAddress.value;
			break;
	}
	modalAddAddress.value = !modalAddAddress.value;
}
...
const handleUpdateAddress = (index) => {
	...
	formAddress.value = objAddress.value[index];
	indexUpdateAddress.value = index;
	modalAddAddress.value = !modalAddAddress.value;
}

const changeCfdiList = (event) => {
	const nsid = event.target.value
	formClient.value._drt_custom_uso_cfdi = nsid
}
		...
		<div class="bg-[#ffffff] w-full rounded-3xl shadow-2xl p-2 md:p-10 lg:p-10">
			<div>
				<div class="flex flex-wrap justify-between">
					<p class="text-4xl md:text-3xl text-[#26245C] font-semibold text-center">Crear un nuevo cliente</p>
					<div class="col-span-2 w-full md:w-auto">
						<button
							...
							@click="handleCreateClient" :disabled="saveClient">
							{{ txtBtnSbe }}</button>
								...
								<div class="flex flex-wrap -mx-3 mb-6">
									<div class="w-full md:w-1/3 px-3 mb-6 md:mb-0">
										...
										<input
											...
											id="grid-last-name" type="text" v-model="formClient.firstname">
									</div>
									<div class="w-full md:w-1/3 px-3 mb-6 md:mb-0">
										<label class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2"
											for="grid-last-name">
										...
										<input
											class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500"
											id="grid-last-name" type="text" v-model="formClient.lastname">
									</div>
									<div class="w-full md:w-1/3 px-3">
										...
										<input
											class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500"
											id="grid-last-name" type="text" v-model="formClient.companyname">
									</div>
								</div>
								<div class="flex flex-wrap -mx-3 mb-6">
									<div class="w-full md:w-1/2 px-3 mb-6 md:mb-0">
										...
										<input
											...
											v-model="formClient.custentity_drt_send_email_invoice">
									</div>
									<div class="w-full md:w-1/2 px-3">
										...
										<input
											class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500"
											id="grid-last-name" type="text" v-model="formClient.phone">
									</div>
								</div>
							</div>
						</div>
				...
				<div class="w-full bg-white border border-gray-200 shadow">
					<ul class="flex flex-wrap text-sm font-medium text-center text-white border-b border-[#26245C] bg-[#26245C]"
						id="defaultTab" data-tabs-toggle="#defaultTabContent" role="tablist">
						<li class="me-2">
							<p class="inline-block p-4">Información de facturación</p>
						</li>
					</ul>
					<div id="defaultTabContent">
						<div class="p-4 bg-white rounded-lg md:p-8" id="about" role="tabpanel" aria-labelledby="about-tab">
							<div class="w-full">
								<div class="flex flex-wrap -mx-3 mb-6">
									<div class="w-full md:w-1/3 px-3">
										...
										<input
											class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500"
											id="grid-last-name" type="text" v-model="formClient._drt_custom_rfc">
									</div>
									<div class="w-full md:w-1/3 px-3 mb-6 md:mb-0">
										<label class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2"
											for="grid-last-name">
										...
										<select
											class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500"
											v-model="formClient._drt_custom_regimen_fiscal">
									...
									<div class="w-full md:w-1/3 px-3">
										...
										<select
											class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500"
											v-model="formClient._drt_custom_uso_cfdi" @change="changeCfdiList($event)">
											<option value="0" selected>Seleccione uno...</option>
								...
								<div class="flex flex-wrap -mx-3 mb-6">
									<div class="w-full md:w-1/2 px-3">
										...
										<select
											class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500"
											v-model="formClient.custentity_metodo_pago">
											<option value="0" selected>Seleccione uno...</option>
									...
									<div class="w-full md:w-1/2 px-3">
										...
										<select
											class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500"
											v-model="formClient.custentity_forma_pago">
											<option value="0" selected>Seleccione uno...</option>
											...
											<table class="min-w-full text-left text-sm font-light">
												...
												<tbody>
													<tr class="border-b text-center items-center"
														v-for="(address, index) in objAddress" :key="index">
														<td class="whitespace-nowrap px-2 py-4">{{ address.calle }}</td>
														<td class="whitespace-nowrap px-2 py-4">{{ address.numero_e }}</td>
														<td class="whitespace-nowrap px-2 py-4">{{ address.numero_i }}
														</td>
														<td class="whitespace-nowrap px-2 py-4">{{ address.colonia }}</td>
														<td class="whitespace-nowrap px-2 py-4">{{ address.ciudad }}</td>
														<td class="whitespace-nowrap px-2 py-4">{{ address.municipio }}</td>
														<td class="whitespace-nowrap px-2 py-4">{{ address.estado }}</td>
														<td class="whitespace-nowrap px-2 py-4">{{ address.pais }}</td>
