...
const handleModalConfimationWithdrawal = () => {
	...
	if(obj_cashbox.value.total <= 0){
		toast.open({
			message: `El total no puede ser menor o igual 0 para realizar un arqueo de caja`,
			type: 'warning'
		})
		return false
	}
	...
	try {
		...
		try {
			...
			if(response.data.code == 200){
				let withdrawals_info = JSON.parse(localStorage.arqueo_cash)
				withdrawals_info.push(obj_cashbox.value)
				localStorage.arqueo_cash = JSON.stringify(withdrawals_info)

				toast.open({
					message: `Arqueo de caja guardada correctamente`,
					type: 'success'
				})
			}else{
				toast.open({
					message: `Hubo un error al guardar el arqueo de caja. Error: ${response.data.msg}`,
					type: 'error'
				})
			}
			}else{
			...
			toast.open({
				message: `Hubo un error al guardar el arqueo de caja. Error: ${error}`,
				type: 'error'
			})
			loader_save.value = false
		}
...
<template>
	<div class="flex bg-[#468aa2] w-full min-h-screen p-4 pt-32 md:pt-24 lg:pt-24">
		...
		<div class="bg-[#ffffff] w-[100%] rounded-3xl shadow-2xl p-2 md:p-10 lg:p-10" v-show="!showTicket" v-else>
			<div class="flex flex-wrap justify-between">
				<p class="text-4xl md:text-3xl text-[#000000] font-semibold text-center w-full md:w-auto">Arqueo de caja</p>
			</div>
			<div class="grid grid-cols-4">
				...
				<div class="p-5 mt-12">
					...
					<div class="flex lg:flex-row md:flex-col mt-5 w-full items-center justify-center">
						<button @click="handleModalConfimationWithdrawal()" class="bg-[#468aa2] text-[#ffffff] w-full mt-5 md:mt-0 lg:mt-0 font-semibold px-8 py-2 rounded-xl float-right ml-5 shadow-inp">Registrar Arqueo</button>
					</div>
				</div>
			</div>
		</div>
		<div class="bg-[#ffffff] w-[100%] rounded-3xl shadow-2xl p-2 md:p-10 lg:p-10" v-show="showTicket && !loader">
			...
			<div class="flex flex-col items-center mt-10" id="ticketsale">
				<div class="w-full md:w-1/2 lg:w-1/3 bg-white shadowCard p-4 items-center text-center">
					...
					<p class="text-lg font-bold mt-5">Arqueo de caja <br> {{ obj_cashbox.id }}</p>
	...
	<div class="relative z-10" aria-labelledby="modal-title" role="dialog" aria-modal="true" v-if="modalConfimationWithdrawal">
		<div class="fixed inset-0 bg-[#000000] bg-opacity-75 transition-opacity" @click="handleModalConfimationWithdrawal()"></div>
		<div class="fixed inset-0 z-10 w-screen" v-if="loader_save">
			<Loader :msg="'Guardando arqueo de caja'"/>
		</div>
						...
						<div class="p-5">
							<div class="flex flex-col mt-3 text-center">
								<h3 class="text-xl font-semibold text-gray-900">
									Esta realizando un arqueo de caja por ${{ Number(obj_cashbox.total).toFixed(2) }}
								</h3>
